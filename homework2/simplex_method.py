"""A Python counterpart to the textbook standard-form simplex routine.

The solver minimizes ``c @ x`` subject to ``A @ x = b`` and ``x >= 0``.  It
expects a feasible initial basis and records the quantities used in each
simplex iteration: reduced costs, entering variable, direction, ratio-test
step length, and leaving variable.
"""
